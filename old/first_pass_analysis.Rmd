...
ggplot(outages, aes(x = log1p(duration_hours))) +
  geom_histogram(binwidth = 1, position = "identity", alpha = 0.6) +
  facet_wrap(~cause, scales = "free_y", labeller = labeller(
    cause = c(
      planned = "Planned Outage",
  ...
  labs(
    title = "Log of Outage Duration by Type",
    x = "Log of Duration",
    y = "Count",
    fill = "Outage Type"
  ) +
...
leaflet(planned_data) %>%
  ...
  addCircleMarkers(
    ...
    stroke = FALSE,
    fillOpacity = 0.8,
    color = ~colorNumeric("YlOrRd", domain = log1p(planned_data$duration_hours))(log1p(duration_hours)),
    label = ~paste0("Duration: ", round(duration_hours, 2), " hrs")
  ) %>%
  addLegend(
    "bottomright",
    pal = colorNumeric("YlOrRd", domain = log1p(planned_data$duration_hours)),
    values = log1p(planned_data$duration_hours),
    title = "log(Duration) (hrs)"
  )
...
leaflet(unplanned_data) %>%
  ...
  addCircleMarkers(
    ...
    stroke = FALSE,
    fillOpacity = 0.8,
    color = ~colorNumeric("YlGnBu", domain = log1p(unplanned_data$duration_hours))(log1p(duration_hours)),
    label = ~paste0("Duration: ", round(duration_hours, 2), " hrs")
  ) %>%
  addLegend(
    "bottomright",
    pal = colorNumeric("YlGnBu", domain = log1p(unplanned_data$duration_hours)),
    values = log1p(unplanned_data$duration_hours),
    title = "Duration (hrs)"
  )
...
```{r}
# Roll each outage-day to its month (year-month)
outage_days <- outage_days %>%
  mutate(month = floor_date(as.Date(days_seq), unit = "month"))

# --- Monthly totals by state ---
monthly_state_summary <- outage_days %>%
  group_by(state, month) %>%
  summarise(total_outage_hours = sum(hours_on_day, na.rm = TRUE), .groups = "drop")

ggplot(monthly_state_summary, aes(x = month, y = total_outage_hours, fill = state)) +
  geom_col(position = "stack") +
  labs(
    title = "Total Outage Hours per Month by State",
    x = "Month",
    y = "Total Outage Hours",
    fill = "State"
  ) +
...
# --- Monthly totals by jurisdiction ---
monthly_jurisdiction_summary <- outage_days %>%
  group_by(jurisdiction, month) %>%
  summarise(total_outage_hours = sum(hours_on_day, na.rm = TRUE), .groups = "drop")

ggplot(monthly_jurisdiction_summary, aes(x = month, y = total_outage_hours, fill = jurisdiction)) +
  geom_col(position = "stack") +
  labs(
    title = "Total Outage Hours per Month by Jurisdiction",
    x = "Month",
    y = "Total Outage Hours",
    fill = "Duke Energy Jurisdiction"
  ) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b %Y") +
  theme_minimal() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
